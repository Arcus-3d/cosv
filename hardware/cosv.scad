...
module paddle(laser=0,bearing_h=bearing_h) {
	difference() {
		union() {
			scale([paddle_x_scale,paddle_y_scale,1]) {
				intersection() {
					translate([0,0,-bvm_r/1.5+arm_w*1.5]) sphere(r=bvm_r/1.5,center=true);
					translate([0,0,bvm_r/2]) cylinder(r=bvm_r/3,h=bvm_r,center=true);
				}
				intersection() {
					translate([0,0,bvm_r/2]) cylinder(r=bvm_r/3+arm_w/2,h=bvm_r,center=true);
					rotate_extrude() translate([bvm_r/3-arm_w/6,0]) scale([1,1.2]) circle(r=arm_w/3,center=true);
				}
			}
		...
		if (0) union() {
			difference() {
				scale([paddle_x_scale,paddle_y_scale,1]) intersection() {
		...
		hull() translate([0,0,-arm_w*2]) rotate([0,-90,0]) {
			translate([arm_w*1.5,0,0]) cylinder(r=bvm_c*1.5-kerf/2,h=bearing_h,center=true);
			translate([arm_w*2.75,-bvm_c,0]) cylinder(r=bvm_c/2-kerf/2,h=bearing_h,center=true);
			translate([arm_w*2.75,bvm_c,0]) cylinder(r=bvm_c/2-kerf/2,h=bearing_h,center=true);
		}
	}
}
