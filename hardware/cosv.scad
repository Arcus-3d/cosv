...
param_motor="bldc"; // [wiper:Ford F150 Wiper Motor,bldc:BLDC Brushless Motor,worm:Worm gear motor,nema23:Nema23 Stepper with 6mm shaft,nema23-635:]

// Is the motor shaft D-Shaped?
param_d_shaft = "auto"; // [zero:Circular,one:D-Shaped,two:Double-D shaped (wiper motor),auto:Let the motor parameter decide]

// Which one would you like to see?
param_part = "arm_l"; // [cam:cam top/bottom (need 2),paddle:paddle for the arms (need 2),base_b:base plate/motor mount (need 1),base_t:top plate/electronics mount (need 1),bldc_motor_standoff:bldc motor has a long pilot on it, which requires a standoff (need 1),bag_mount:Thin arms to support the bag ends and keep it from wandering (need 2),arm_l:Arm,supply_cover:Power Supply cover,chest_bar:Chest bar,flow_sensor_for_pcb:Flow sensor body for use with a PCB (pitot style),flow_sensor_venturi_for_pcb:Flow sensor body for use with a PCB (venturi style),flow_sensor_for_pcb_no_pocket:Flow sensor test body,flow_sensor_cover_oled:Flow sensor cover for use with embedded OLED,flow_sensor_cover:Flow sensor cover,flow_sensor_test:Test apparatus for the flow sensor,bearing_bushing:parts used within the other FFF parts,bearing_washer:parts used within the other FFF parts]
