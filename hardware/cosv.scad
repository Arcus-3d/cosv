...
//laser_base_front_standalone(); // base front
...
//base_front_standalone(); // top plate/electronics mount.  Need 1.
...
//rj45_jack();
module rj45_jack() {
	square([16-kerf,13.25-kerf],center=true);
}
...
module laser_base_front_standalone() {
...
module base_front_standalone(h=material_t) {
	difference() {
...
module base_front_pi(h=material_t,layer=2) {
	...
	if (layer == 0) {
		difference() {
			hull() for (x=[-1,1]) for (y=[-1,1]) translate([x*(housing_h/2+bvm_c*3-material_t/2),y*(bvm_l/2-h),h/2]) cylinder(r=extra,h=h,center=true);
			translate([0,0,h/2]) cube([pi_x,pi_y2,h+extra],center=true);
		}
	}	
	if (layer == 1) {
		difference() {
			base_front_model(h=h);
			for (x=[-1,1]) for (y=[-1,1]) translate([x*(pi_bolt_x/2),y*(pi_bolt_y/2),h/2]) cylinder(r=cover_bolt_r,h=h+extra,center=true);
			hull() for (x=[-1,1]) for (y=[-1,1]) translate([x*(pi_bolt_x/2),y*(pi_bolt_y/2-10),h/2]) cylinder(r=cover_bolt_r,h=h+extra,center=true);
			hull() for (x=[-1,1]) for (y=[-1,1]) translate([x*(pi_bolt_x/2+10),y*(pi_bolt_y/3),h/2]) cylinder(r=cover_bolt_r,h=h+extra,center=true);
		}
	}
}
