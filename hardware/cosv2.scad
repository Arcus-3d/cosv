...
// Project repository: 		https://github.com/Arcus-3d/cosv
...
//laser_base_end_right(slots=0); // base, top plate
//laser_base_end_left(slots=0); // base, top plate
//laser_base_front_analog(); // base front
...
//paddle(); // paddle for the arms.  Need 2
//base_mount_right(); // base plate/motor mount.  Adjust the bolt pattern you need below first.  Need 1.
//base_mount_left(); // base plate opposite motor mount. Need 1.
//base_end_left(); // base end plate/bag mounts.  Need 1, top bag end diameter.
//base_plate(); // top plate/electronics mount.  Need 1.
//base_front_analog(); // top plate/electronics mount.  Need 1.
//base_front_pi(layer=1); // top plate/electronics mount.  Need 1.
...
//bearing_bushing(); // parts used within the other FFF parts
//bearing_washer(); // parts used within the other FFF parts
//laser_base_mount_left(); // base, bottom plate
//laser_base_mount_right(); // base, bottom plate
...
module cosv_assembly_view(explode=0,cam_angle=0) {
	...
	if (1) translate([0,0,-bvm_l/2-material_t-explode*4]) base_end_right(slots=0);
	...
	if (1) translate([0,0,cam_h/2+material_t*3+clearance*2]) rotate([0,180,0]) mirror([1,0,0]) base_mount_left();
	if (1) translate([0,0,bvm_l/2+explode*3]) rotate([0,180,0]) base_end_left();
	if (1) translate([0,0,+bvm_l/2+material_t+explode*4]) rotate([0,180,0]) base_end_left(slots=0);
	if (1) translate([0,y_pos,bvm_l/2-material_t-65/2]) %battery();
...
module base_mount_model(h=material_t*2,explode=0) {
	difference() {
		...
		hull () for (x=[0,1]) for (y=[-1,1]) translate([x_pos-x*bvm_c*7-material_t/2,y_pos+y*(housing_h/2-bvm_c*3-x*bvm_c*2),h/2]) cylinder(r=6/2,h=h+extra,center=true);
		hull () for (x=[0,1]) for (y=[-1,1]) translate([-x_pos+x*bvm_c*7+material_t/2,y_pos+y*(housing_h/2-bvm_c*8-x*bvm_c*2),h/2]) cylinder(r=6/2,h=h+extra,center=true);
	}
}
...
module base_mount_left(h=material_t*2,explode=0) {
	difference() {
		...
		hull() for (x=[1,2]) for (y=[-1,1]) translate([cam_l/2.35*x,-cam_y_offset-y*(6-2)/2-6/2+extra,h/2]) cylinder(r=2/2,h=h+extra,center=true);
		#hull() for (x=[1,2]) for (y=[-1,1]) translate([cam_l/2.15*x,-cam_y_offset-y*(6-2)/2+6/2-extra,h/2]) cylinder(r=2/2,h=h+extra,center=true);
	}
}
