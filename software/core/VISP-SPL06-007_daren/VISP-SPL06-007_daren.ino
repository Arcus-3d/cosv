...
void loopVenturiVersion(float *P, float *T)
{
  ...
  if ( dataReady ) {
    ...
    if (command == "calibrate" ) {
      hwSerial.print(F("state"));
      hwSerial.print(F(","));
      hwSerial.println(F("calibrate"));
    ...
    if (command == "run")
    {
      ...
      hwSerial.print(F("state"));
      hwSerial.print(F(","));
      hwSerial.println(F("run"));
    }
    if (command == "rate")
    {
      int rate = atoi(param[1]);
      hwSerial.print(F("rate"));
      hwSerial.print(F(","));
      hwSerial.println(rate);
      analogWrite(M_PWM_1,rate*8);
      analogWrite(M_PWM_2,rate*8);
    }
  }
  switch (runState)
  {
    ...
    case RUNSTATE_RUN:
      ...
      if (lastSampleTime)
      {
        tidalVolume = tidalVolume + volumeSmoothed *(sampleTime - lastSampleTime)/60 - 0.01; // tidal volume is the volume delivered to the patient at this time.  So it is cumulative.
      }
      if (tidalVolume < 0.0)
      {
        tidalVolume = 0.0;
